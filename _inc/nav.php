              <?php while ($row = $result->fetch_object()) : ?>
                <?php if($row->showmenu && !$row->subpagina) : ?>
                  <li>
                    <a href="<?php echo ($row->anchor) ? '#'.$row->slug : "page?slug=".$row->slug; ?>" title="<?php echo $row->titulo; ?>"><?php echo $row->titulo; ?></a>
                    <?php 
                              ...
                              if($fetchSubmenu->num_rows) : 
                                $submenuTpl = '<ul class="submenu">';
                                  while ($row = $fetchSubmenu->fetch_object()) :
                                    $submenuTpl .= '<li><a href="'.(($row->anchor) ? '#'.$row->slug : "page?slug=".$row->slug).'" title="'.$row->titulo.'" target="'.(($row->anchor) ? '_self' : '_blank').'">'.$row->titulo.'</a></li>';
                                  endwhile;
                                $submenuTpl .= '</ul>';
                                echo $submenuTpl;
                              endif; 
                          ...
                          if($fetchSubmenu = $conn->prepare("SELECT * FROM `$row->slug` ORDER BY id ASC")) :
                            if($fetchSubmenu) :
                              ...
                              while($fetchSubmenu->fetch()) :
                                  ...
                                  $rowURL = $rowURL;
                                  $rowTEXT = $rowTEXT;
                                  $submenuTpl .= '<li><a href="single?post='.$row->slug.'&id='.$rowID.'" title="'.$rowTITULO.'">'.$rowTITULO.'</a></li>';
                              endwhile;
                              $submenuTpl .= '</ul>';
                              echo $submenuTpl;
                            endif; 
