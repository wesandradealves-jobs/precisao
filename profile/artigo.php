<?php
    ...
    if(!isset($_GET['id'])){
        if(isset($_POST['update'])) :
            $stmt = $conn->prepare("INSERT artigos (`titulo`, `url`, `text`, `headers`, `slug`) VALUES (?, ?, ?, ?, ?)");
            $titulo = $_POST['titulo'];
        ...
        $id = $_GET['id'];
        $stmt = $conn->prepare("SELECT `titulo`, `url`, `text`, `headers`, `slug` FROM `artigos` WHERE `artigos`.`id` = '".$id."'");
        if($stmt){
            $stmt->execute();
        ...
        if(isset($_POST['update'])) :
            ...
            $stmt = $conn->prepare("UPDATE artigos SET `titulo` = ?, `url` = ?, `text` = ?, `headers` = ?, `slug` = ? WHERE `artigos`.`id` = '".$id."'");
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <?php if(isset($url) && $url!=='') : ?> 
                                            ...
                                            <p><small>Arquivo atual: <a class="<?php echo (substr($url, -3) == 'png' || substr($url, -3) == 'jpg' || substr($url, -3) == 'gif' || substr($url, -3) == 'bmp') ? 'lightbox' : ''; ?>" target="_blank" href="uploads/<?php echo (isset($url)) ? $url : ''; ?>"><?php echo (isset($url)) ? $url : ''; ?></a> <a href="<?php echo "../_inc/delete.php?source=artigo-thumbnail&id=".$id."&uid=".$uid."&file=".$url; ?>" title="Deletar arquivo atual"><small>(Remover arquivo atual)</small></a></small></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="form-group">
